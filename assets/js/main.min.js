jQuery(function ($) {
	...
	jQuery(document).on('click', '#add_new_sub', function(e){
		...
		new_sub += '<div class="col-md-1" style="margin-top: 20px"><div class="btn-group" role="group" aria-label="Basic example">';
		new_sub += '<button type="button" id="remove_sub" class="btn upl btn-primary btn-block"><i class="far fa-trash-alt"></i></button> </div>';
		new_sub += '</div></div></div></div>';
